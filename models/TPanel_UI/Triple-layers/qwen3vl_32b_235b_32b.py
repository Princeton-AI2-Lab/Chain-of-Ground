...
class Qwen3VL235B32BTripleMethod:  
    def __init__(self,           
        ...
        """            
        三层 Qwen 组合模型: Qwen-32B初检 + Qwen-235B修正 + Qwen-32B最终验证  
            
        Args:            
            qwen_model: Qwen-235B模型名称(Dashscope格式)  
    ...
    def call_openrouter_api(self, model, prompt, image, max_retries=3):  
        ...
        for attempt in range(max_retries):  
            try:  
                ...
                self.debug_print(f"OpenRouter API调用失败 (尝试 {attempt + 1}/{max_retries}): {e}")  
    ...
    def ground_with_qwen_32b_initial(self, instruction, image):  
        """第一层: Qwen3-VL-32B初检"""  
        self.debug_print("=== 第一层: Qwen3-VL-32B初检 ===")  
    ...
    Where x,y are integers in [0, 1000] range."""  
        ...
        self.debug_print(f"Qwen-32B初检响应: {response}")  
...
x,y integers [0,1000].""" 
        ...
        response = self.call_dashscope_api(messages, model_name=self.qwen_32b_model)  
        self.debug_print(f"Qwen-32B最终修正响应: {response}")  
    ...
    def ground_only_positive(self, instruction, image):  
        ...
        # 第二层: Qwen-235B修正  
        qwen_235b_pixel_point, qwen_235b_response = self.refine_with_qwen(  
            instruction, resized_image, qwen_32b_pixel_point, qwen_32b_initial_response, is_correct=False  
        )  
        
        if qwen_235b_pixel_point is None:  
            self.debug_print("第二层修正失败, 回退到第一层结果")  
            qwen_235b_pixel_point = qwen_32b_pixel_point  
        ...
        final_pixel_point, qwen_32b_final_response = self.refine_with_qwen_32b(  
            instruction, resized_image, qwen_32b_pixel_point, qwen_235b_pixel_point,  
            qwen_32b_initial_response, qwen_235b_response  
        )  
        
        # 第三层失败回退到第二层  
        if final_pixel_point is None:  
            self.debug_print("第三层修正失败, 回退到第二层结果")  
            final_pixel_point = qwen_235b_pixel_point  
        ...
        annotated_final = self.plot_annotated_circle(annotated_final, qwen_235b_pixel_point, 10, True, 100)  
        annotated_final = self.plot_annotated_circle(annotated_final, final_pixel_point, 15, None, 150)  
        ...
        return {  
            ...
            "raw_response": {  
                ...
                "qwen_235b_refined": qwen_235b_response,  
