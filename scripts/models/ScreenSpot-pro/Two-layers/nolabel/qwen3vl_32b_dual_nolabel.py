...
class Qwen3VL32BDualMethod:  
    ...
    def refine_with_qwen(self, instruction, resized_image, initial_pixel_point, initial_response, is_correct=False):
        """Layer 2: Qwen3-VL correction - use only coordinates, not annotated image"""
        ...
        prompt = f"""**CRITICAL: You MUST output in the exact format specified below. Do NOT provide explanations or natural language responses.**  
        
    ## Task  
    {instruction}  
    
    ## Previous Detection  
    A previous system detected coordinates at [{x_1000}, {y_1000}] (in 0-1000 range).  
    
    ## Your Mission  
    1. Verify if the coordinates [{x_1000}, {y_1000}] point to the correct target for: "{instruction}"  
    2. Determine the CORRECT center coordinates:  
    - If previous coordinates ARE correct → output SAME coordinates [{x_1000}, {y_1000}]  
    - If previous coordinates are NOT correct → find correct element and output its coordinates  
    
    ## Output Format (MANDATORY)  
    You MUST output ONLY the following structure wrapped in <tool_call> tags:  
    <tool_call>  
    {{"name": "computer_use", "arguments": {{"action": "left_click", "coordinate": [x_normalized, y_normalized]}}}}  
    </tool_call>  
    
    Where:  
    - x_normalized, y_normalized are integers in range [0, 1000]  
    - NO additional text before or after the <tool_call> tags  
    - NO explanations or descriptions  
    
    If you cannot find the target element, output:  
    <tool_call>  
    ...
    def ground_only_positive(self, instruction, image):
        ...
        self.debug_print("Initial check completed")
        time.sleep(1)  # Avoid API rate limiting
        
        # Layer 2: call Qwen correction with original resized image and coordinates
        final_pixel_point, qwen_refined_response = self.refine_with_qwen(  
            instruction, resized_image, qwen_pixel_point, qwen_response, is_correct=False  
        )  
